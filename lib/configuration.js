...
var adAuth = require('4front-ad-auth');
...
module.exports = function(app, localInstance) {
  ...
  if (localInstance === true) {
    ...
    // Configure ActiveDirectory as the security provider
    app.settings.identityProviders = [
      {
        name: 'ActiveDirectory',
        default: true,
        authenticate: require('4front-ad-auth')({
          ldapUrl: process.env.FF_LDAP_URL,
          ldapBaseDN: process.env.FF_LDAP_BASE_DN
        }).authenticate
      }
    ];
  }
